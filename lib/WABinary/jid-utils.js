...
const jidToLid = jid => {
    ...
    if (split.length === 2 && split[1] === 's.whatsapp.net') {
        ...
        if (cleanUser.length === 14 && /^(67|68|69)\d{12}$/.test(cleanUser)) {
            return `${user}@lid`;
        }
    }
    return jid;
};
exports.jidToLid = jidToLid;
const getBotJid = (jid) => {
...
const storeLidPnMapping = (lid, pn) => {
...
exports.storeLidPnMapping = storeLidPnMapping;

const getPnForLid = (lid) => {
...
const isTcTokenExpired = (timestamp) => {
    ...
    const TC_TOKEN_NUM_BUCKETS = 4; // 28 days
