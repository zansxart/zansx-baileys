    ...
    const handleEncryptNotification = async (node) => {
        ...
        if (from === WABinary_1.S_WHATSAPP_NET) {
            ...
            const result = await (0, Utils_1.handleIdentityChange)(node, {
                meId: authState.creds.me ? authState.creds.me.id : undefined,
                meLid: authState.creds.me ? authState.creds.me.lid : undefined,
    ...
    const handleGroupNotification = (participant, child, msg) => {
        ...
        switch (child === null || child === void 0 ? void 0 : child.tag) {
            ...
            case 'revoked_membership_requests':
                ...
                break;
            default:
            // console.log("zansxart-DEBUG:", JSON.stringify({ ...child, content: Buffer.isBuffer(child.content) ? child.content.toString() : child.content, participant }, null, 2))
        }
    };
        ...
        switch (nodeType) {
            case 'privacy_token':
                ...
                for (const { attrs, content } of tokenList) {
                    ...
                    const storageJid = (0, WABinary_1.getLidForPn)(jid) || jid;
                    try {
                        await authState.keys.set({
                            tctoken: {
                                [storageJid]: {
                                    token: Buffer.from(content),
                                    timestamp: attrs.t || Math.floor(Date.now() / 1000).toString()
                                }
                            }
                        });
                        logger.debug({ jid, storageJid }, 'stored privacy token in authState');
                    } catch (err) {
                        logger.error({ jid, err }, 'failed to store privacy token');
                    }
                }
                break;
            case 'newsletter':
        ...
        try {
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    await decrypt();
                    if (msg.key && msg.key.remoteJid && msg.key.id && msg.message && messageRetryManager) {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        if (((_a = msg === null || msg === void 0 ? void 0 : msg.messageStubParameters) === null || _a === void 0 ? void 0 : _a[0]) === Utils_1.NO_MESSAGE_FOUND_ERROR_TEXT) {
                            const unavailableNode = (0, WABinary_1.getBinaryNodeChild)(node, 'unavailable');
                            const unavailableType = unavailableNode === null || unavailableNode === void 0 ? void 0 : unavailableNode.attrs.type;
                            ...
                            const messageAge = Math.floor(Date.now() / 1000) - (+msg.messageTimestamp);
                            if (messageAge > 1209600) { // 14 days
                            ...
                            if ((0, WABinary_1.isJidStatusBroadcast)(msg.key.remoteJid)) {
                                const messageAge = Math.floor(Date.now() / 1000) - (+msg.messageTimestamp);
                                if (messageAge > 24 * 60 * 60) { // status broadcast expires in 24 hours
                        ...
                        if (!(0, WABinary_1.isJidGroup)(msg.key.remoteJid) && (0, WABinary_1.isLidUser)(msg.key.remoteJid)) {
                            ...
                            if (targetJid) {
                                (0, WABinary_1.storeLidPnMapping)(msg.key.remoteJid, targetJid);
