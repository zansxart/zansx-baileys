...
const makeMessagesRecvSocket = (config) => {
    const { logger, retryRequestDelayMs, placeholderResendRequestDelayMs, maxMsgRetryCount, getMessage, shouldIgnoreJid } = config;
    ...
    const handleMessage = async (node) => {
        ...
        if ((0, WABinary_1.getBinaryNodeChild)(node, 'unavailable') && !encNode) {
            ...
            // ONAH FIX: Fire-and-forget — jangan block handleMessage
            // Pesan akan datang via resend nanti, tidak perlu nunggu 5+ detik
            requestPlaceholderResend(key).catch(err => 
                logger.debug({ err }, 'placeholder resend failed')
            );
            logger.debug('received unavailable message, acked and requested resend from phone (non-blocking)');
            return;
        }
        else {
    ...
    const requestPlaceholderResend = async (messageKey) => {
        ...
        if (placeholderResendRequestDelayMs) {
            await (0, Utils_1.delay)(placeholderResendRequestDelayMs);
        }
        if (!placeholderResendCache.get(messageKey === null || messageKey === void 0 ? void 0 : messageKey.id)) {
