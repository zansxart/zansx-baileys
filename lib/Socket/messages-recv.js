...
const makeMessagesRecvSocket = (config) => {
    ...
    const processedMessagesCache = config.processedMessagesCache || new node_cache_1.default({
    ...
    const handleMessage = async (node) => {
        ...
        if (msg && msg.key && msg.key.id) {
            ...
            if (processedMessagesCache.get(msg.key.id)) {
                logger.debug({ id: msg.key.id }, 'message already processed, skipping delivery');
                return;
            }
            // Mark as processed
            processedMessagesCache.set(msg.key.id, true);
        }
