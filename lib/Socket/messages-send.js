...
const makeMessagesSocket = (config) => {
    const { logger, linkPreviewImageThumbnailWidth, generateHighQualityLinkPreview, options: axiosOptions, patchMessageBeforeSending, cachedGroupMetadata, albumMessageItemDelayMs = 0, enableRecentMessageCache = true, maxMsgRetryCount = 5, } = config;
    const sock = (0, newsletter_1.makeNewsletterSocket)(config);
    const { ev, authState, processingMutex, signalRepository, upsertMessage, query, fetchPrivacySettings, sendNode, groupMetadata, groupToggleEphemeral, } = sock;
    const messageRetryManager = enableRecentMessageCache !== false ? new Utils_1.MessageRetryManager(logger, maxMsgRetryCount) : null;
    ...
    const sendReceipt = async (jid, participant, messageIds, type) => {
        const mappedJid = (0, WABinary_1.getLidForPn)(jid);
        if (mappedJid) {
            jid = mappedJid;
        }
        jid = (0, WABinary_1.jidToLid)(jid);
        if (participant) {
            const mappedParticipant = (0, WABinary_1.getLidForPn)(participant);
            if (mappedParticipant) {
                participant = mappedParticipant;
            }
            participant = (0, WABinary_1.jidToLid)(participant);
        }
        const node = {
            tag: 'receipt',
            attrs: {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, AI = false }) => {
        ...
        if (participant && participant.jid) {
            const mappedParticipant = (0, WABinary_1.getLidForPn)(participant.jid);
            if (mappedParticipant) {
                participant.jid = mappedParticipant;
            }
            participant.jid = (0, WABinary_1.jidToLid)(participant.jid);
        }
        ...
        await authState.keys.transaction(async () => {
            ...
            if (AI && isPrivate) {
                ...
                const hasBotNode = Array.isArray(additionalNodes) && additionalNodes.some(
                    item => item?.tag === 'bot' && item?.attrs?.biz_bot === '1'
                );
                if (hasBotNode) {
            ...
            const is1on1Send = !isGroup && !participant && !isStatus && !isNewsletter && !isPeerMessage;
            if (is1on1Send) {
                const tcTokenJid = destinationJid;
                ...
                if (tcTokenBuffer && tcTokenBuffer.length && (0, WABinary_1.isTcTokenExpired)(existingTokenEntry === null || existingTokenEntry === void 0 ? void 0 : existingTokenEntry.timestamp)) {
                ...
                if (tcTokenBuffer && tcTokenBuffer.length) {
                    stanza.content.push({
                        tag: 'tctoken',
                        attrs: {},
                        content: tcTokenBuffer
                    });
                    logger.debug({ jid: destinationJid }, 'appended tctoken to outgoing message');
                }
            }
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' && 'album' in content && content.album) {
                ...
                return (0, Utils_1.generateWAMessageFromContent)(jid, msg.message, {
                    userJid,
                    messageId: msgId,
                    ...options
                });
