...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, upsertMessage, creds, keyStore, logger, options, getMessage }) => {
                        ...
                        if (retryResponse) {
                            ...
                            setTimeout(() => {
                                if (typeof upsertMessage === 'function') {
                                    (0, exports.cleanMessage)(webMessageInfo, meId);
                                    Promise.resolve(upsertMessage(webMessageInfo, 'notify', {
                                        requestId: response.stanzaId
                                    })).catch(error => {
                                        logger.debug({ error, messageId: webMessageInfo?.key?.id }, 'failed to upsert placeholder resend response');
                                    });
                                    return;
                                }
