...
class MessageRetryManager {
    constructor(logger, maxMsgRetryCount) {
        ...
        this._baseKeys = new lru_cache_1.LRUCache({
            max: 1024,
            ttl: 15 * 60 * 1000,
            ttlAutopurge: true
        });
    ...
    saveBaseKey(addr, msgId, baseKey) {
        this._baseKeys.set(`${addr}:${msgId}`, baseKey);
    }
    hasSameBaseKey(addr, msgId, baseKey) {
        const stored = this._baseKeys.get(`${addr}:${msgId}`);
    ...
    deleteBaseKey(addr, msgId) {
        this._baseKeys.delete(`${addr}:${msgId}`);
    }
    _keyToString(key) {
